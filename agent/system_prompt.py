...
def build_system_prompt_parts(agent: Any, system_message: Optional[str] = None) -> Dict[str, str]:
    ...
    # Ghost desktop control — cross-platform (Win/Mac/Linux)
    _ghost_desktop_tools = {
        "desktop_capture", "desktop_click", "desktop_type", "desktop_paste",
        "desktop_hotkey", "desktop_press", "desktop_scroll", "desktop_drag",
        "desktop_window_list", "desktop_window_focus",
    }
    if _ghost_desktop_tools & set(agent.valid_tool_names):
        import platform as _platform
        _sys = _platform.system()
        stable_parts.append(
            f"## Ghost Desktop Control ({_sys})\n"
            "You have full desktop control. Operating principles:\n"
            "1. **Always screenshot first** — call `desktop_capture` before clicking or typing "
            "to understand the current screen state and find exact coordinates.\n"
            "2. **Natural movement** — `desktop_click` moves the mouse along a bezier curve. "
            "Never teleport. Always use human=true (default).\n"
            "3. **Natural typing** — `desktop_type` types at realistic WPM with variance. "
            "For long text or Unicode/CJK, use `desktop_paste` instead.\n"
            "4. **Verify after action** — take another screenshot after clicking/typing "
            "to confirm the action worked.\n"
            "5. **Window management** — use `desktop_window_list` to find windows, "
            "`desktop_window_focus` to bring them to front.\n"
            "6. **Coordinate system** — coordinates are in screen pixels from top-left (0,0). "
            "Use the Vision analysis from `desktop_capture` to identify element positions.\n"
            "7. **Patience** — add small waits between actions when needed "
